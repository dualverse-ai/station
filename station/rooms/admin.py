...
# station/rooms/admin.py
"""
Implementation of the Administrative Counter Room for the Station.
...
_ADMIN_COUNTER_HELP = """
**Welcome to the Administrative Counter.**
...
* Requesting external information: Agents are not allowed to access external data or literature unless the specification allows it. The Station runs independently of the external world.
* Requesting new supervisors: If your supervisor is absent **or** is not responding you for more than 7 ticks, please continue with your research activities and assume your research direction is approved. The Station will automatically assign a new supervisor when appropriate.
...
To display this help message again, issue `/execute_action{help admin}`.
"""

class AdminCounter(BaseRoom):
    def __init__(self):
        super().__init__(constants.ROOM_ADMIN)
        self.log_dir = os.path.join(
            constants.BASE_STATION_DATA_PATH,
            constants.ROOMS_DIR_NAME,
            constants.ADMIN_COUNTER_SUBDIR_NAME
        )
    ...
    def _get_agent_request_ids(self, agent_data: Dict[str, Any], consts) -> List[Any]:
        request_ids = agent_data.get(consts.AGENT_HUMAN_INTERACTION_IDS_KEY)
        if isinstance(request_ids, list):
            return list(request_ids)

        legacy_id = agent_data.get(consts.AGENT_HUMAN_INTERACTION_ID_KEY)
        if legacy_id:
            return [legacy_id]
        return []

    def _agent_has_pending_requests(self, agent_data: Dict[str, Any], consts) -> bool:
        if agent_data.get(consts.AGENT_AWAITING_HUMAN_INTERVENTION_FLAG, False):
            return True
        return bool(self._get_agent_request_ids(agent_data, consts))
    ...
    def get_room_output(self,
        ...
        if self._agent_has_pending_requests(agent_data, room_context.constants_module):
            request_ids = self._get_agent_request_ids(agent_data, room_context.constants_module)
            request_ids_display = ", ".join(str(req_id) for req_id in request_ids) if request_ids else "N/A"
            # Return the standard help message first via super()
            base_output = super().get_room_output(agent_data, room_context, current_tick)
            status_message = (
                f"\n\n**Current Status:** You have active request(s) for human assistance (Request ID(s): {request_ids_display}).\n"
    ...
    def _get_specific_room_content(self,
        ...
        if self._agent_has_pending_requests(agent_data, room_context.constants_module):
            ...
            request_ids_display = ", ".join(str(req_id) for req_id in request_ids) if request_ids else "N/A"
            return (
                f"You are at the Administrative Counter. Your Request ID(s) {request_ids_display} are pending human review.\n"
                "You can continue normal activities while awaiting response."
            )

        return (
            "You are at the Administrative Counter. This is where you can log a request for a Human Assistant.\n"
        ...
        try:
            ...
            for agent_file in os.listdir(agents_dir):
                ...
                try:
                    ...
                    if self._agent_has_pending_requests(agent_data, constants):
                        request_ids = self._get_agent_request_ids(agent_data, constants)
                        if request_ids:
                            self.pending_requests[agent_name] = request_ids
    ...
    def get_pending_requests_summary(self, active_agents=None):
        ...
        Returns:
            dict: {
                ...
                'agent_request_map': {'Agent1': [1, 2], 'Agent2': [3]}  # Agent to request ID mapping
            }
        """
        ...
        request_ids = []
        for req_ids in filtered_requests.values():
            if isinstance(req_ids, list):
                for req_id in req_ids:
                    if isinstance(req_id, int):
                        request_ids.append(req_id)
            elif isinstance(req_ids, int):
                request_ids.append(req_ids)
    ...
    def handle_action(self,
        ...
        if agent_data.get(consts.AGENT_STATUS_KEY) == consts.AGENT_STATUS_GUEST:
            actions_executed.append(f"Action '{action_command}' denied: The Administrative Counter is only available to Recursive Agents.")
        ...
        if action_command == consts.ACTION_REQUEST_HUMAN:
            ...
            try:
                ...
                request_ids = self._get_agent_request_ids(agent_data, consts)
                request_ids.append(request_id)
                delta_updates = {
                    consts.AGENT_AWAITING_HUMAN_INTERVENTION_FLAG: True,
                    consts.AGENT_HUMAN_INTERACTION_ID_KEY: request_id,
                    consts.AGENT_HUMAN_INTERACTION_IDS_KEY: request_ids
                }
                if room_context.agent_manager.update_agent_fields(agent_name, delta_updates):
                    ...
                    agent_data[consts.AGENT_HUMAN_INTERACTION_IDS_KEY] = request_ids
            ...
            actions_executed.append(
                f"Action '{action_command}' is not recognized in the Administrative Counter. "
    ...
    def save_request_resolution(self, request_id: int, human_response: Optional[str], resolution_reason: str, resolution_tick: int) -> bool:
        ...
        try:
            if not file_io_utils.file_exists(self.log_file_path):
                print(f"AdminCounter: Log file not found at {self.log_file_path}")
                return False
            ...
            for req in requests:
                if req.get('request_id') == request_id:
                    ...
                    found_request = True
                    print(f"AdminCounter: Found and updating request ID {request_id}")
                updated_requests.append(req)

            if not found_request:
                print(f"AdminCounter: Warning - Request ID {request_id} not found in log")
                return False
            ...
            print(f"AdminCounter: Saved resolution to request log for ID {request_id}{response_msg}")
            return True

        except Exception as e:
            print(f"AdminCounter: Error saving resolution for request ID {request_id}: {e}")
            return False
