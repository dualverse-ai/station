...
For a multi-node run, update the `RESEARCH_STORAGE_BASE_PATH` field in the task
bundle before initialization:

```text
example/research_alpha_evolve/kissing/constant_config.yaml
````

Set it to a shared directory that is accessible from all nodes.

Finally, initialize and start the Station on the **main node**:

```bash
station init example/research_alpha_evolve/kissing "My Station"
```
...
```bash
cp example/research_alpha_evolve/kissing/research_tasks_stage2.md station_data/rooms/research/research_task.md
```
