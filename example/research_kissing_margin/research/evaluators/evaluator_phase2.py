...
"""
Evaluator for phase 2 of the kissing-number direction-margin task.
"""
...
class Task1Evaluator(ResearchTaskEvaluator):
    """
    Evaluator for kissing configurations in TARGET_DIMENSION with N >= TARGET_COUNT.
    """
    ...
    def evaluate_submission(
        ...
        eval_id: str = None,
        author: str = None
    ) -> Tuple[bool, Any, Any, Tuple[float, int, float]]:
        """
        Score submitted direction vectors by total overlap loss after normalization.

        Lower primary score is better. The custom sort key makes accepted larger
        configurations rank above accepted smaller configurations.
        """
        try:
        ...
        if score <= 0.0 and self._looks_exact(raw_array):
        ...
        if score <= 0.0:
            n = int(metrics["n"])
            message = (
                f"Accepted configuration: {n} direction vectors in dimension {self.dim}; "
        ...
        return True, float(score), details, self._build_sort_key(score, int(metrics["n"]))
    ...
    def get_task_description(self) -> str:
        return (
            f"Phase 2 kissing direction-margin loss in dimension {self.dim} "
            f"with N>={self.target_count}"
        )

    def _build_sort_key(self, score: float, n: int) -> Tuple[float, int, float]:
        # Research Center ranks larger sort keys first.
        if score <= 0.0:
            return (1.0, int(n), -float(score))
        return (0.0, 0, -float(score))
    ...
    def _validate_vectors(self, vectors: np.ndarray) -> None:
        ...
        if vectors.ndim != 2:
            raise AssertionError(
                f"Expected 2D array with at least {self.target_count} rows and {self.dim} columns; "
        ...
        if num_vectors < self.target_count:
            raise AssertionError(
                f"Expected at least {self.target_count} vectors, but got {num_vectors}."
            )
    ...
    def _save_scored_config(
        ...
        try:
            ...
            save_data = dict(
                ...
                target_count=int(metrics["n"]),
                minimum_count=self.target_count,
                dimension=self.dim,
                phase=PHASE_LABEL,
