...
from env import create_train_env, create_test_env
...
def select_action(
    ...
    """Select action using the policy network (sampling)."""
...
def select_action_greedy(
    ...
    """Select action using the policy network (greedy argmax)."""
    # Add batch dimension for single observation
    observation_batched = observation[None, ...]
    done_batched = done[None, ...] if done is not None else None
...
def evaluate_agent(
    ...
    Args:
        greedy: If True, use argmax action selection. If False, use sampling.
    """

    action_selector = select_action_greedy if greedy else select_action
    ...
    def run_episode(key, level_id):
        ...
        def step_fn(carry, unused):
            ...
            # Select action (greedy or sampling based on mode)
            action, _, _, new_rnn_state = action_selector(
...
def run_single_seed_training(funcs, hparams, seed, config, trial_timeout):
    ...
    # CRITICAL: Update global constants before JIT compilation
    global NUM_ENVS, UNROLL_LENGTH
    ...
    # Use TEST dataset for evaluation (unfiltered-test split)
    test_env = create_test_env(time_limit=config['time_limit'])
    ...
    # Track solve rates for trailing average
    recent_solve_rates = []
    window_size = 10  # For 0.99 weighted average or simple moving average
    ...
    for update in range(NUM_UPDATES):
        ...
        recent_solve_rates.append(float(solve_rate))
        if len(recent_solve_rates) > window_size:
        ...
        # Collect data for compact logging every 5M steps
        updates_per_5M = int(5_000_000 / (config['num_envs'] * config['unroll_length']))
        if update % updates_per_5M == 0 or update == NUM_UPDATES - 1:
            timesteps = (update + 1) * config['num_envs'] * config['unroll_length']
            trailing_avg = sum(recent_solve_rates) / len(recent_solve_rates) if recent_solve_rates else 0.0
            step_markers.append(f"{timesteps/1000000:.1f}M")
            return_values.append(f"{float(mean_return):.2f}")
            solve_values.append(f"{trailing_avg:.3f}")
            # Print trailing solve rate every 5M steps
            print(f"[{timesteps/1000000:.1f}M steps] Trailing solve rate (avg of last {len(recent_solve_rates)}): {trailing_avg:.3f}")
    
    # Final evaluation - both sampling and greedy
    key, eval_key_sampling, eval_key_greedy = jax.random.split(key, 3)

    print("\nRunning final evaluation (sampling mode)...")
    results_sampling = evaluate_agent(test_env, network, params, eval_key_sampling, config['time_limit'], greedy=False)
    final_solve_rate_sampling = results_sampling['solve_rate']

    print("Running final evaluation (greedy mode)...")
    results_greedy = evaluate_agent(test_env, network, params, eval_key_greedy, config['time_limit'], greedy=True)
    final_solve_rate_greedy = results_greedy['solve_rate']
    ...
    print(f"\nFinal test solve rate (sampling): {final_solve_rate_sampling:.3f}")
    print(f"Final test solve rate (greedy): {final_solve_rate_greedy:.3f}")

    # Return sampling solve rate as primary score (for backward compatibility)
    return final_solve_rate_sampling, final_solve_rate_greedy, params, opt_state