...
def _coerce_test_state(arr, expected_n):
    x = np.asarray(arr)
    if x.ndim != 2:
        raise ValueError(f"Expected rank-2 array from test(), got shape {x.shape}")
    if x.shape == (expected_n, DIM):
        return x
    if x.shape == (DIM, expected_n):
        return x.T
    raise ValueError(f"Expected shape ({expected_n}, {DIM}) or ({DIM}, {expected_n}), got {x.shape}")
...
def _run_test_mode(submission_module):
    ...
    shared_data_dir = os.path.join("storage", "shared", "data")
    os.makedirs(shared_data_dir, exist_ok=True)
    print(f"SUBMISSION_UUID: {submission_uuid}")

    if not isinstance(test_result, np.ndarray):
        print("TEST_STATE_SAVED: 0")
        print("TEST_SCORE_MODE: n.a.")
        print("=== Test Mode Complete ===")
        return

    try:
        state = _coerce_test_state(test_result, N)
        out_path = os.path.join(shared_data_dir, f"test_state_{submission_uuid}.npz")
        np.savez(out_path, sphere_centers=np.asarray(state))
        print("TEST_STATE_SAVED: 1")
        print(f"TEST_STATE_PATH: {out_path}")
        print("TEST_SCORE_MODE: evaluator")
    except Exception as e:
        print("TEST_STATE_SAVED: 0")
        print(f"TEST_VERIFY_ERROR: {e}")
        print("TEST_SCORE_MODE: n.a.")
