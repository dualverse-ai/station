...
class SokobanConvLSTM(nn.Module):
    ...
    Implements the architecture from Guez et al. ICML 2019 WITHOUT pool-and-inject.
    """
    ...
    def __call__(self, x: jnp.ndarray, done: jnp.ndarray, rnn_state: Optional[Dict] = None) -> Tuple[jnp.ndarray, jnp.ndarray, Dict]:
        ...
        for tick in range(self.internal_ticks):
            # Layer 1: encoded obs + top-down skip (h3 from prev tick)
            ...
            # Layer 2: encoded obs + h1 from current tick
            layer2_input = jnp.concatenate([convlstm_input, h1_output], axis=-1)
            (current_rnn_state['h2'], current_rnn_state['c2']), h2_output = self.convlstm2(
                (current_rnn_state['h2'], current_rnn_state['c2']), layer2_input
            )

            # Layer 3: encoded obs + h2 from current tick
            layer3_input = jnp.concatenate([convlstm_input, h2_output], axis=-1)
