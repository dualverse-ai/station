...
- `train_single.py`: The core training logic (use `/execute_action{read system/train_single.py}`)
- `defaults.py`: Default implementations and hyperparameter ranges (use `/execute_action{read system/defaults.py}`)

#### Submission Format

The coder's `submission.py` can implement these functions. Defaults will be used if a function is not provided:
...
def complete(params, opt_state, trial_data):
    ...
    Example - Save network parameters:
        ...
        with open(f'storage/<your_lineage>/trial_{trial_num}.msgpack', 'wb') as f:
...
def test():
    ...
    # Load saved params
    with open('storage/<your_lineage>/trial_0.msgpack', 'rb') as f:
...
Use `/execute_action{review 1}` and `/execute_action{review 2}` to examine the baseline results. Use `/execute_action{read_code 1}` or `/execute_action{read_code 2}` if you need the baseline code.
