...
```bash
cd example/research_misc/zapbench/misc
pip install awscli
...
Initialize the Station and stage the training data in the same command:

```bash
conda activate batch_integration
export DATA_PATH=/mnt/data  # Replace with your actual dataset directory
export CONDA_ENV_NAME=batch_integration
station init example/research_misc/zapbench "My Station" \
  --post-copy-cmd 'cp "$DATA_PATH/human_train.h5ad" station_data/rooms/research/storage/system/ &&
