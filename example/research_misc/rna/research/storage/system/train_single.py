...
# Dataset configurations (matching main.py)
DATASET_CONFIGS = {
    "APA": {
        "d_output": 1,
...
def create_batches(sequences, labels, batch_size, shuffle_key=None):
    ...
    if padded_size > n_samples:
        ...
        if labels.ndim == 1:
            # Sequence-level regression (APA)
            label_pad = jnp.zeros(pad_size, dtype=labels.dtype)
        else:
            # Multi-label (Modif, PRS)
...
def compute_metric(task_type: str, metric: str, predictions, targets, dataset=None):
    ...
    if task_type == "regression":
        if metric == "r2":
            ...
            if np.isnan(corr):
                ...
                return 0.0
            return corr
    elif task_type == "multilabel_classification" and metric == "auc_roc":
        ...
        try:
            ...
            return 0.0
    elif task_type == "multilabel_regression" and metric == "r2":
    ...
    elif task_type == "multiclass_classification" and metric == "accuracy":
...
def run_single_dataset_training(funcs, hparams, seed, dataset_name, trial_timeout):
    ...
    # Add dataset-specific info to hparams for network creation
    hparams_with_config = hparams.copy()
    hparams_with_config.update({
    ...
    # Initialize network
    network = funcs['create_network'](hparams_with_config)
    ...
    train_step = make_train_step(network, optimizer, compute_loss_fn, hparams_with_config)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for i, (batch_X, batch_y) in enumerate(zip(batched_val_X, batched_val_y)):
        ...
        val_metric = compute_metric(task_type, metric, all_predictions, all_targets, dataset_name)
    ...
    # Create diagnostics
    cap_exhausted = (time.time() - start_time > trial_timeout)
    diagnostics = {
    ...
    return best_val_metric, best_params, opt_state, diagnostics